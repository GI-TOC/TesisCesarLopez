...
Asignacion<- function(Sq_pos,Habilidad,n,TP,N.op){
  Tiempo <- order(TP, decreasing = TRUE)
  Dispo<-list()
  Prog<-sample(0,N.op,replace = TRUE)
  for (i in 1:n){
    Quien=which(Habilidad[,Tiempo[i]]==1)
    Dispo[[i]]=Quien 
  }
  # Asigno primer operador segun heuristica
  a=sample(1:length(Dispo[[1]]),1)
  Prog[Dispo[[1]][a]]<- Prog[Dispo[[1]][a]]+TP[Tiempo[1]]
  Sq_pos[3,which(Sq_pos[2,]==Tiempo[1])]<-Dispo[[1]][a]
  
  # Asigno el resto de operadores según heurística
  for (j in 2:n){
    Cond<-as.numeric(Dispo[[j]]%in% Sq_pos[3,])
    Sum_Cond<-sum(Cond)
    Sum_Cond
    if (Sum_Cond>=1){
      if (0%in%Cond){
        #Escojo aletoriamente el menos ocupado
        Cond2 <-as.numeric(Cond%in%1)
        Firmes<- Cond2[-which(Cond2==1)]
        auxi<-Dispo[[j]]
        auxi<-auxi[-which(Cond2==1)]
        Dispo[[j]]<-auxi
        c=sample(1:length(Dispo[[j]]),1)
        Prog[Dispo[[j]][c]]<- Prog[Dispo[[j]][c]]+TP[Tiempo[j]]
        Sq_pos[3,which(Sq_pos[2,]==Tiempo[j])]<-Dispo[[j]][c]
        Sq_pos[3,]
      }else{
        # Escojo de todos el menos ocupado
        Cuales<-Dispo[[j]]
        T_Menos<-Prog[Cuales]
        d<-order(T_Menos)
        e<-which(Cuales[d[1]]==Dispo[[j]])
        Prog[Dispo[[j]][e]]<- Prog[Dispo[[j]][e]]+TP[Tiempo[j]]
        Sq_pos[3,which(Sq_pos[2,]==Tiempo[j])]<-Dispo[[j]][e]
      }
    }else{
      #Escojo aleatoriamente cualquiera.
      b=sample(1:length(Dispo[[j]]),1)
      Prog[Dispo[[j]][b]]<- Prog[Dispo[[j]][b]]+TP[Tiempo[j]]
      Sq_pos[3,which(Sq_pos[2,]==Tiempo[j])]<-Dispo[[j]][b]
      Sq_pos[3,]
    }
  }
  return(Sq_pos)
...
write.table(Mejor_Cmax,file="Resultados_SSA1.txt")